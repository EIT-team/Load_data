		...
		% filesize, position of eventtable, headerlength, etc. 	
			...
			t = reshape(char(t),sz,[]);
                ...
                elseif strcmp(HDR.TYPE,'BDF') && any(strmatch('Status',HDR.Label)),
                    ...
                    %old line [t,c] = fread(HDR.FILE.FID,inf,[int2str(HDR.AS.SPR(HDR.BDF.Status.Channel)*3),'*uint8'],HDR.AS.bpb-HDR.AS.SPR(HDR.BDF.Status.Channel)*3);
                    %new line - added
                    [t,c] = fread(HDR.FILE.FID,inf,[int2str(HDR.AS.SPR(HDR.BDF.Status.Channel)*3),'*uint8=>uint8'],HDR.AS.bpb-HDR.AS.SPR(HDR.BDF.Status.Channel)*3);
			if (c>HDR.NRec*HDR.SPR*3)
				% a hack to fix a bug in Octave Ver<=3.0.0
        	                t = t(1:HDR.NRec*HDR.SPR*3);
        	        end;
                    
                    t_mat=reshape(t,3,length(t)/3)';
                    HDR.BDF.ANNONS=uint32(t_mat(:,1))*uint32(2^0)+uint32(t_mat(:,2))*uint32(2^8)+uint32(t_mat(:,3))*uint32(2^16);
