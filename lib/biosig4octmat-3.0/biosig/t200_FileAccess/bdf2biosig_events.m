...
case 1,	
    ...
    % JAMES ADDED BIT - MOVED INTO THIS CASE
...
case 4,
	...
	t = bitand(HDR.BDF.ANNONS,2^16-1);
	t(~~bitand(HDR.BDF.ANNONS,2^16)) = 0; 
    % we dont need double here as code only takes positive changes anyway
	ix2 = diff(double([0;bitand(t,(2^16-1))]));	% labels 
	POS = [find(ix2>0)];
	TYP = [t(ix2>0)];
...
case 5,
	...
	t(~~bitand(HDR.BDF.ANNONS,2^16)) = 0; 
	ix2 = diff(uint32([0;bitand(t,2^16-1)]));	% labels 
	POS = [find(ix2)];
 	TYP = [t(ix2>0); t(find(ix2<0)-1)+hex2dec('8000')];
...
HDR.EVENT.TYP = double(TYP(ix));


%%JAMES ADDED BIT - WE ARE FINISHED WITH THIS NOW
clear ix2

%%%% BDF Trigger and status 
t = bitand(HDR.BDF.ANNONS,hex2dec('00ffff')); 
ix = diff(int32([0;t])); %have to conver to signed here as we can have negative diff
HDR.BDF.Trigger.POS = find(ix);
HDR.BDF.Trigger.TYP = double(t(HDR.BDF.Trigger.POS)); 			
